...
class DataHandle:
    ...
    args:: extmodel_filename : Filename of the external model


   optional args::
           param_names_dict : dictionary containing parameter number as key and
                                name of the parameter as value
    ...
    def __init__(self, extmodel_filename,
                       ...
                       ratio_mode= False,
                       param_names_dict={'parameter_1':'beta'},
                       verbosity=1):
        #super(, self).__init__()
        ...
        self.mindexcols_ext = multindex_cols_ext
    ...
    def read_csv_pandas(self, verbosity=1):
        ...
        if self.ratio_mode ==False :
            print('Shape of imported reference model dataframe: ', str(self.df_ref.shape))
            self.multindex_ref = self.df_ref.index
            self.multindex_names_ref = list(self.multindex_ref.names)
        print('Shape of imported extended model dataframe: ', str(self.df_ext.shape))
    ...
    def calculate_ratio_by_redshifts(self,redshfit_list,normalize = True, pos_norm = 2):
        empty_arr =   [[] for ii in range(len(self.noise_names))]
        self.matrix_ratios_dict= dict(zip(self.noise_names, empty_arr))
        for z in np.sort(redshfit_list):
           matrix_z = self.calculate_ratio_data(z,normalize,pos_norm, False)
        ...
        self.z_requested = redshfit_list
    
    def calculate_ratio_data(self, z,normalize = True,pos_norm = 2,SAVING = True):
        try:
    ...
    def calculate_data_split(self, n_train=2, n_vali = 0, n_test=1, n_splits=1,
                             ...
                             manual_split = False, test_indices= None,**kwargs):
        ...
        if n_train+n_test > (self.size_fullspace):  #
            raise ValueError("n_train is larger than total full sample space")
        ...
        if manual_split == False :
            ...
            for ii in range (n_splits):
                ...
                nb_param = int(len(self.fullspace)/len(self.z_vals))
                
                redshift_index = test[0]//nb_param #Index of the redshift
                test_mod_redshift = [tt%nb_param for tt in test ]
                train_origin = [ii for ii in range(0,nb_param) if ii not in test_mod_redshift ] # should add the extrema
                train_origin = [ii + redshift_index * nb_param for ii in range(0,nb_param) if ii not in test_mod_redshift ] # should add the extrema

                train = shuffle(train_origin)


                n_redshift = n_train//nb_param
                print(n_train," training vectors taken on",n_redshift,"redshift")
                redshift = [zz for zz in range(0,len(self.z_vals))]
                redshift = shuffle(redshift)
                redshift = redshift[:n_redshift]                      
                print("redshift used :",redshift)

                for zz in redshift:
                    train_zz = [ii + zz*nb_param  for ii in train_origin  ]

                    train = np.concatenate((train,train_zz))


                self.test_splitdict[ii] = test
                shuffled = shuffle(train)
                self.shuffled = shuffled
                self.train_splitdict[ii] = shuffled [:n_train]
                self.vali_splitdict[ii] = shuffled [n_train:n_train+n_vali]
    ...
    def data_split(self, split_index=0, thinning=None, apply_mask=False, mask=[], **kwargs):
        ...
        for kki in self.matrix_ratios_dict.keys():
            ...
            for dli in self.learn_sets:
                ...
                mask_true=maskcopy[::thinning]  ##apply thinning, if set to None, there is no thinning
                ## copy of mask to avoid modifying orginal mask after iterations
                setattr(self, 'masked_'+self.features_str, self.fgrid[mask_true]) ##apply mask also to feature grid and save as masked_+...

                matrixdata = matrixdata[:,mask_true]  ## apply mask and thinning to feature space (k-grid)
