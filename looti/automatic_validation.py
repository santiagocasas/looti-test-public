                    ...
                    if interp_type =='GP':
                        for ll in ll_tests:
                            for Y_noise in noise_tests :
                                print("====== PCA Noise parameter, Length parameter: ",Y_noise, ll)
                                Op = dcl.LearningOperator('PCA', ncomp=ncp, interp_type=interp_type, gp_length=ll)
        ...
        if operator == "LIN":
            ...
            for ncp in range(1,emulation_data.train_size+1):
                ...
                if interp_type =='GP':
                    for ll in ll_tests:
                        for Y_noise in noise_tests :
                            print("====== PCA Noise parameter: ",Y_noise)
                            try:
